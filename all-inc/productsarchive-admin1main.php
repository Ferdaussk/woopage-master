...
function productsarchive_admin_enqueue(){
	if (isset($_GET['page']) && $_GET['page'] === 'productsarchive_single_sk') {
		...
		// noUiSlider cdn
		wp_enqueue_style('productsarchive-nouislider-style', plugins_url('/admin-assets/nouislider/nouislider.css',__FILE__), null, '1.0', 'all');
		wp_enqueue_script('productsarchive-nouislider-script', plugins_url('/admin-assets/nouislider/nouislider.js',__FILE__), array('jquery'), '15.5.0', true);
		// For noUiSlider custom assets
		wp_enqueue_script('productsarchive-admin-nouislider-custom-script', plugins_url('/admin-assets/js/nouislider-custom.js',__FILE__), array('jquery'), '1.0', true);
		wp_enqueue_style('productsarchive-admin-nouislider-custom-style', plugins_url('/admin-assets/js/nouislider-custom.css',__FILE__), null, '1.0', 'all');
		// Tab assets
	...
	// 
		if(current_user_can('manage_options')){
			add_submenu_page(
				...
				'WooPage Master',
				'manage_options',
				'productsarchive_single_sk',
				'productsarchive_settings_cb',
				12
			);
		}
		// 
...
add_action( 'activated_plugin', function ( $plugin ) {
	if ( plugin_basename( __FILE__ ) == $plugin ) {
		wp_redirect( admin_url( 'admin.php?page=productsarchive_single_sk' ) );
		die();
	}
} );
...
function productsarchive_tab_section(){
	...
	<div class="productsarchive_tab_items">
		...
		<div class="productsarchive_save_btn"><input type="button" class="productsarchive_settings_reset" onclick="checkReset(); resetSlider();" value="Reset All" id="resetButton"></div>
	</div>
	<?php
}
#################---####### Settings start
...
function productsarchive_lb_en_gallery_cb(){
	...
	echo '<label class="toggle-switch"><input type="checkbox" id="productsarchive-enable-product-single-page" name="productsarchive-enable-product-single-page" value="true" '.checked('true',$productsarchive_lb_en_gallery_value,false).'><span class="toggle-slider"></span></label>';
}
// Product single page styles
function productsarchive_button_position_cb(){
	echo '<div id="wpwrap">';
		echo '<div class="popup-button" title="'.__('Please check (Use our style?) option.').'">'.__('Go Templates').'</div>';
	echo '</div>';
	$sections = array(
		'style1' => 'Demo 1',
		'style2' => 'Demo 2',
	...
	echo '<div class="popup-overlay" id="popupOverlay">';
		echo '<div class="popup-content">';
			echo '<span id="closePopup">'.__('Close').'</span>';
				...
				foreach ($sections as $section_id => $section_title):
					echo '<section id="'.esc_attr($section_id).'">';
							...
							echo '<button class="install-button" data-installed="false" onclick="simulateLoading(this)">';
									...
									echo '<span class="install-text">'.__('Install').'</span>';
									echo '<span class="uninstall-text">'.__('Uninstall').'</span>';
							echo '</button>';
					echo '</section>';
				endforeach;
			echo '</div>';
...
$default_value = sanitize_text_field(get_option('productsarchive-nouislider'));
function productsarchive_try_nouislider_fld(){
  ...
  ?>
  <div class="woopm_noUi_Slider" id="productsarchive-nouislider"></div>
